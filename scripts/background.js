...
chrome.commands.onCommand.addListener((command) => {
    ...
    if (command === 'run-dark-mode') {
        ...
        chrome.tabs.query({currentWindow: true, active: true}, function (tabs) {
            ...
            if (tabUrl.slice(-4) === '.pdf')
                runScriptWithBadge(activeTab.id);
        });
    }
  });
...
callbackUpdated = function (tabid, changeInfo, tab) {
    ...
    // onUpdated fires for many reasons; URL may be undefined.
    const updatedUrl = (changeInfo && typeof changeInfo.url === 'string') ? changeInfo.url : undefined;
    const tabUrl = (tab && typeof tab.url === 'string') ? tab.url : undefined;
    const urlToCheck = updatedUrl || tabUrl;

    if (!urlToCheck)
        return;

    let url = urlToCheck.slice(-4);
